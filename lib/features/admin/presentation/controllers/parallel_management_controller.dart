...
class ParallelManagementController extends ChangeNotifier {
  final IAdminRepository repository;

  ParallelManagementController({required this.repository});

  final List<CycleModel> _cycles = [];
  final List<ParallelModel> _parallels = [];
  String _searchQuery = '';
  ...
  List<ParallelModel> get parallels => List.unmodifiable(_parallels);
  ...
  List<ParallelModel> get filteredParallels {
  ...
  String getCycleName(String cycleId) {
    return _cycles.firstWhere(
      ...
      orElse: () => const CycleModel(id: '', name: '', level: 0),
    ).name;
  }
  ...
  Future<bool> toggleStatus(ParallelModel parallel) async {
    return saveParallel(
